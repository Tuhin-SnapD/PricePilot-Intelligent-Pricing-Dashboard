// src/components/PricingOptimization.jsx
...
const PricingOptimization = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (withForecast) {
          ...
          const optMap = optimization.reduce((acc, { product_id, optimized_price }) => {
  ...
  const filtered = optData.filter(d =>
      ...
      <div className="min-h-screen flex flex-col bg-gradient-to-b from-black to-gray-800 text-white">
        ...
        <div className="mx-4 mt-4 mb-2 bg-gray-900 rounded-lg px-4 py-2 flex items-center">
          ...
          <div className="flex items-center space-x-3">
            ...
            <span className="text-base font-semibold">Pricing Optimization</span>
          </div>

          {/* spacer so the controls sit on the far right */}
          <div className="flex-1" />

          {/* Right: Toggle, Search, Category, Filter */}
          <div className="flex items-center space-x-4">
            ...
            <button className="flex items-center bg-gray-800 border border-gray-700 px-3 py-1 rounded text-white hover:bg-gray-700">
            ...
            <table className="min-w-full text-sm text-left text-gray-900 bg-white">
              <thead className="bg-gray-800 text-white">
                <tr>
                  ...
                  <th className="p-3">Optimized Price</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {filtered.map((d, idx) => {
                  ...
                  const diff = d.optimized_price != null
                    ? sell - d.optimized_price
                    : null;
                  ...
                  return (
                    <tr key={d.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="p-3">${isNaN(cost) ? '—' : cost.toFixed(2)}</td>
                      <td className="p-3">${isNaN(sell) ? '—' : sell.toFixed(2)}</td>
                      <td className="p-3 text-green-600 font-semibold">
                        {d.optimized_price != null ? `$${d.optimized_price.toFixed(2)}` : '—'}
                      </td>
